# LexFrontier｜法学前沿论文雷达

LexFrontier 是一个按需调用、公开归档的法学前沿论文 Skill。它检索近期中英文成果，优先关注法律与 AI、算法治理、计算法学、法律科技以及真正具有法律贡献的交叉学科研究，用普通人也能理解的中文解释，并将每次日报按日期保存到 GitHub。
...
skill/track-legal-frontiers/SKILL.md        平台中立的工作流
skill/track-legal-frontiers/references/     来源、筛选和报告规范
skill/track-legal-frontiers/scripts/        候选发现与归档脚本
tests/                                      标准库测试
```
...
其他支持通用 `SKILL.md` 约定的 Agent，可以直接加载 `skill/track-legal-frontiers/SKILL.md`。

调用示例：

```text
请使用 $track-legal-frontiers 生成并归档今天的法学前沿论文日报。
```
...
可选环境变量：`OPENALEX_API_KEY` 用于提高 OpenAlex API 容量；`LEXFRONTIER_MAILTO` 会在 Crossref 请求中加入联系邮箱。
